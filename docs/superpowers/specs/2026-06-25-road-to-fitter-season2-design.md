...
> **탭 IA 개편(확정)**: 식단/수면/물/영양제를 들어내면 기록(체중+메모)·요약(체중그래프+1RM)이 둘 다 얇아지고 "체중" 중심으로 중복된다. 따라서 **요약 탭을 제거하고 그 콘텐츠를 기록 탭으로 흡수**해 **3탭(홈 / 운동 / 기록)**으로 단순화한다.
> - **홈** → 오늘의 추가운동 진행 + 최근 체중
> - **운동** → 날짜별 운동(공용+개인) 기록
> - **기록** → 체중 입력 + 체중 그래프 + 1RM + 메모 (요약 흡수)
...
| **기록 (`/daily`) — 통합 탭** | 위에서 아래로 4섹션: ① 체중 입력 ② 체중 그래프(`WeightChart`, 전체 연속·동적 범위) ③ 1RM(`OneRMSection`) ④ 메모. 시즌1 daily의 수면·식단·OCR·매크로·물·영양제·당가공·식단횟수·운동여부 토글 전부 제거. 자동저장 패턴 유지 |
| **요약 (`/summary`) — 제거** | 라우트·페이지 삭제. `WeightChart`·`OneRMSection` 컴포넌트는 **유지**해 기록 탭에서 import. `MacroChart`·`WeeklyStats`·주차 선택·`PROGRAM_START/END`는 폐기 |
...
| **하단 네비 (`BottomNav`)** | **4탭→3탭**: 홈 / 운동 / 기록. 요약 탭 제거. 라벨/아이콘 미세 조정 |
...
4. 기록 탭 통합 (체중 입력 + 체중 그래프 + 1RM + 메모 — 요약 흡수)
5. 요약 라우트 제거 + 하단 네비 3탭(홈/운동/기록)
6. 홈 + 헤더 개편
